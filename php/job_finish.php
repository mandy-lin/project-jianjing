...
$data = mysqli_query($con,"SELECT DISTINCT j_no,j_date,j_type,c_name,jt_team,jt_teamphone FROM job j 
            ...
            WHERE j.j_status='已完成' and c.c_name like '%$keyword%' 
...
<div id='content'>
    <div class="card"> 
        ...
        <caption>已完成派工單</caption>
            ...
            if(mysqli_num_rows($data) > 0){
                ...
                echo '<tr>
                      ...
                      <td>'. $job['j_type'].'</td>
                      <td >
                      <input class="btn bt-3" type="button" value="詳細 " onclick='.'location.href='.'"job_finished_detail.php?id='.$job['j_no'].'">
                      </td>
                    </tr>';
        ...
        <?php     
        if ($page > 1) {
                echo '<input  type="button" class="page page-1" value="第一頁 " onclick=location.href="job_finish.php?page=1&keyword='.$keyword.'">'; 

               echo '<input  type="button" class="page" value="上一頁 " onclick=location.href="job_finish.php?page='. ($page - 1) . '&keyword='.$keyword.'">';
               }          
                  for ($i = 1; $i <= $pages; $i++) 
                  { 
                    if ($i == $page) {
                  ...
                  if ($page < $pages) 
                    ...
                    {
                    echo '<input  type="button" class="page" value="下一頁 " onclick=location.href="job_finish.php?page='. ($page + 1) . '&keyword='.$keyword.'">';
                    echo '<input class="page page-2" type="button" value="最後一頁 " onclick=location.href="job_finish.php?page='. ($pages) . '&keyword='.$keyword.'">';
                    }         
