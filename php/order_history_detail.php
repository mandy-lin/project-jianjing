...
$data1 = mysqli_query($con,"SELECT DISTINCT c_name,c_address , c_cname, c_cphone,p_date ,p_comp_date,total ,o_date,o_status,o_actual_date,p_start_date,p_end_date FROM `quote`q join client c on(q.c_no=c.c_no) join order_recode o on(q.price_no=o.price_no) where q.price_no='$id'");
...
<p class="card-header">訂單狀態：<?php echo $rd['o_status'];?></p>
    ...
    <p class="card-date">訂單日期:<?php echo $rd['o_date'];?><br>
    預計施工:<?php echo $rd['p_start_date'];?>
    ~<?php echo $rd['p_comp_date'];?><br>
    實際施工:<?php echo $rd['p_end_date'];?>
    ~<?php echo $rd['o_actual_date'];?></p>
...
<tr > 
    ...
    <p class="card-end">
    <input type="button" id="back" value="返回" onclick="history.back()" class="end-btn bt-3 bt-r"></p>
  </div>
  </div>
