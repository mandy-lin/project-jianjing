...
$fix_no=substr(date("ymdHis"),2,9);
    
$fix=mysqli_query($con,"INSERT INTO `fix`(`fix_no`,  `f_star_date`, `f_statue`, `price_no`, `m_pno`) VALUES  ($fix_no,'$p_start_date','0','$id','$mpno')");
    
$insert=mysqli_query($con,"INSERT INTO `job`( `j_no`,`j_status`,`j_type`,`j_date`,`j_start_date`, `j_comp_date`,`j_meno`, `j_menof`, `j_notice`, `jt_no`,`fix_no`,`price_no`) VALUES ($jno,'未完成','臨時維修','$j_date','$p_start_date', '$p_comp_date','$j_meno','$j_menof','$j_notice','$jt_no','$fix_no','$id')");
...
$fixwork= mysqli_query($con,"INSERT INTO `fix_item`(`fix_no`,`f_work_item`) VALUES ($fix_no,'$j_work[$i]')"); 
 }


    
if($insert and $fix and $sql and $fixwork){echo 'insert';}else{echo 'no'.mysqli_error($con);}  
//新增後跳轉
if($insert){
echo "<script language=javascript>; 
location.href='fix_temporary.php';
</script>";
}
}
...
<div id='content'>
    ...
    ?>
     <p>臨時維修派工單</p>   
        ...
        <input type="date" id="p_start_date" name="p_start_date"   class=" date-input" onchange="handler(event);">
        <br>
        ...
        <table class='table' cellspacing="0">
            ...
            <tbody  id="myTable">
                ...
                <tr>
                    ...
                    <td colspan="4"><input class='table-input' type="text"  name="j_work[]"></td>
                </tr>
            </tbody>
            <tr>
                <th>備註(不收費)</th>
        ...
        <form id="form2" name="form2" method="post" action="" > <div class="card-end">    
        <input type="submit" name="insert" onclick="return  Insert(this.form)" value="新增" class="end-btn bt-3 bt-g"></div></form>
